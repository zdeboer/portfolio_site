-- Creates the DB + tables your 8-Track PHP code expects.
-- NOTE: This schema is inferred from the PHP queries in src/projects/8-Track.
...
-- PLAYLIST TRACKS
-- Your code inserts `added_by`, but one UI check reads `user_id` from the track row.
-- To keep both working without changing PHP, include BOTH and sync them via trigger.
